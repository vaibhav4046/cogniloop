...
- 2026-06-08 perf: extract question card into memoized QuestionCard component — the active session question card (question text, type/difficulty/mode badges, copy button, read-aloud) re-rendered on every answer keystroke because it lived inline in the Session component body; extracting it into a memo component with stable props (round, copiedQ, onCopy via useCallback([rounds]), modeName) makes KaTeX math parsing and badge rendering skip entirely during typing; only re-renders when a new round starts (key prop remount), copiedQ flips, or mode changes; C-key keyboard shortcut DRYed to call the same copyQuestion callback

- 2026-06-08 additional curriculum topic: expanded "Convolutional networks" stub in ML Fundamentals Models from a bare label to a full topic — convolution operation (kernel, stride, padding, output size formula W'=(W−F+2P)/S+1), weight sharing and translation equivariance, parameter count (F×F×C_in+1)×C_out, feature maps, receptive field growth through stacked layers, standard Conv→BN→ReLU→Pool sequence, max pooling vs global average pooling, architecture progression LeNet-5→AlexNet (ReLU, dropout, GPU)→VGG (uniform 3×3)→GoogLeNet/Inception (1×1 bottlenecks, parallel branches)→ResNet (residual/skip connections x+F(x) solve vanishing gradient for 50–152+ layer networks)→EfficientNet (compound scaling), depthwise separable convolutions (MobileNet — factor ≈1/C_out+1/F² computation savings), batch normalization, transfer learning from ImageNet-pretrained features, data augmentation; a 3–5 question ML Fundamentals topic (Stanford CS231n) tested via output dimension calculation, parameter counting, explaining translation equivariance, why residual connections enable deeper networks, and depthwise separable conv efficiency — previously a bare stub in the ML Fundamentals Models pack
...
- 2026-06-08 additional curriculum topic: expanded "Rotational motion and moment of inertia" stub in JEE Physics from a bare 5-word label to full NCERT Class 11 Ch 7 coverage — rigid body definition; rotational kinematics (θ, ω, α) and the three kinematic equations mirroring linear motion (ω=ω₀+αt, θ=ω₀t+½αt², ω²=ω₀²+2αθ); torque τ = r × F = rF sinθ with right-hand rule direction; moment of inertia I = Σmᵢrᵢ² as rotational analog of mass; parallel axis theorem I = Icm + Md²; perpendicular axis theorem Iz = Ix + Iy (laminar bodies only); standard MI values: ring MR², disc ½MR², solid cylinder ½MR², hollow cylinder MR², solid sphere ⅖MR², hollow sphere ⅔MR², rod about centre ML²/12, rod about end ML²/3; Newton's 2nd law for rotation τ_net = Iα; rotational KE = ½Iω²; angular momentum L = Iω, dL/dt = τ_net; conservation of angular momentum (figure skater: I↓ → ω↑ to keep L constant); rolling without slipping v_cm = Rω, total KE = ½Mv²(1 + k²/R²); incline acceleration a = g sinθ/(1 + I/MR²) — solid sphere > disc > hollow cylinder; full translational–rotational analog table (m↔I, F↔τ, v↔ω, p↔L, F=ma↔τ=Iα, W=Fs↔W=τθ); a 3–5 question JEE Main chapter (NCERT Class 11 Ch 7) tested via MI calculation using axis theorems, τ=Iα problems, angular momentum conservation, and rolling KE/incline problems — previously a bare stub in the JEE Physics pack

- 2026-06-08 additional curriculum topic: expanded "Social structures and demographics" stub in MCAT Psych/Soc from a bare label to a full topic — social stratification theories (Marx bourgeoisie/proletariat/false consciousness, Weber's three-dimensional class/status/party model, Davis-Moore functionalist thesis and its critiques), social class indicators (income vs wealth distribution, SES composite), social mobility (vertical/horizontal, intergenerational/intragenerational, absolute vs relative, U.S. intergenerational elasticity ≈ 0.45), poverty (absolute vs relative, poverty traps, Gini coefficient), race/ethnicity as social constructs with real consequences, racialization, individual vs institutional discrimination, implicit bias and IAT, stereotype threat (Steele & Aronson), intersectionality (Kimberlé Crenshaw — overlapping identities produce unique forms of privilege and oppression), gender stratification and wage gap, ageism, social determinants of health (SDOH) and the SES health gradient (Whitehall studies), racial health disparities and weathering hypothesis (Geronimus), healthy immigrant effect and acculturation, sick role (Parsons — four criteria) and its critiques, medicalization, social institutions (family/education/religion/government/medicine), demographic transition model (Stages 1–4); a 4–6 question MCAT Psych/Soc section topic tested via stratification theory identification, intersectionality application, sick role criteria, structural vs individual discrimination, and social determinants affecting a patient case — previously a bare label in the pack

- 2026-06-08 additional curriculum topic: expanded "Recursion" stub in AP CSA from a bare label to full Unit 10 coverage — base case vs recursive case (StackOverflowError if base case missing), call stack mechanics (descent = stack frames accumulate, unwind = frames pop on return), classic examples with traces: factorial (factorial(4) descent 4→3→2→1→0 then unwind returns 1→2→6→24), Fibonacci (fib(5) makes 15 total calls, exponential call tree), power, array sum, String reversal; recursive binary search (O(log n) stack frames); recursive vs iterative tradeoffs (elegant but uses O(depth) stack memory vs O(1) iterative); AP CSA exam question types: (1) trace — hand-execute each call and return to find the final value, (2) count — how many times is the method called for a given input, (3) identify — circle the base case and recursive case, (4) predict output including print statements in both descent and unwind phases, (5) write — implement a recursive method ensuring the base case is reachable and the recursive argument strictly reduces toward it; a 4–6 question AP CSA Unit 10 chapter previously a bare stub

- 2026-06-08 a11y: add aria-live="polite" announcement to HistoryView session search filter — when search is active, screen readers now hear "N of M sessions match" or "No sessions match '…'" on each keystroke, matching the identical stable live-region pattern already used by TemplatesView (added 2026-05-29); the sr-only div is always in the DOM so the announcement is reliable across screen reader / browser combinations

- 2026-06-07 additional curriculum topic: expanded "Electrostatics and current electricity" stub in NEET Physics from a bare 5-word label to full NCERT Class 12 Ch 1 + Ch 3 coverage — electrostatics: Coulomb's law, superposition, Gauss's law (spherical shell / line charge / plane sheet), electric potential and E = −dV/dr, equipotential surfaces, dipole torque and field, conductor properties, parallel-plate capacitor with/without dielectric, series/parallel combinations and energy stored; current electricity: drift velocity, Ohm's law, resistivity temperature dependence (metals vs semiconductors), EMF and internal resistance (terminal voltage), Joule heating, Kirchhoff's KCL and KVL, Wheatstone bridge balance condition, potentiometer null-method for EMF comparison and internal resistance; a 4–6 question NEET Physics topic (NCERT Class 12 Ch 1 and Ch 3) — previously a bare 5-word stub

- 2026-06-07 additional curriculum topic: expanded "Information theory" stub in ML Fundamentals Math & Stats from a 4-word label to a full topic — Shannon entropy H(X) = −Σ p(x) log₂ p(x), binary entropy, joint/conditional entropy, chain rule, mutual information I(X;Y) (feature selection, information bottleneck), KL divergence D_KL(P||Q) with Gibbs' inequality, forward vs reverse KL (mean-seeking vs mode-seeking), cross-entropy loss connection (minimising H(p,q) = minimising D_KL), Jensen-Shannon divergence (symmetric, bounded, GAN training link), perplexity as exp(H) for language model evaluation, VAE ELBO derivation (reconstruction − D_KL regulariser), Shannon source-coding theorem (compression bound = H bits), channel capacity; a foundational ML interview topic and Stanford CS229/CS224n subject previously a bare 4-word stub in the ML Fundamentals Math & Stats pack

- 2026-06-07 additional curriculum topic: expanded "Differential equations" stub in JEE Mathematics from a bare label to full NCERT Class 12 Ch 9 coverage — order and degree definitions (degree undefined for non-polynomial DEs), formation by eliminating n arbitrary constants; three solution methods: (i) variable separable f(y)dy = g(x)dx, (ii) homogeneous DE dy/dx = F(y/x) via substitution y = vx, (iii) linear first-order dy/dx + P(x)y = Q(x) via integrating factor e^(∫P dx); applications — exponential growth/decay dN/dt = kN → N = N₀e^(kt), Newton's law of cooling dT/dt = −k(T−Tₛ), and curve-geometry problems; a 2–4 question JEE Main chapter (NCERT Class 12 Ch 9) tested via order/degree identification, solving variable-separable and homogeneous DEs, applying the integrating-factor method, and finding particular solutions satisfying initial conditions — previously a bare stub in the JEE Mathematics pack

- 2026-06-07 a11y: add aria-label to "GitHub →" external links in NavBar and Landing mini-nav so screen readers announce "Cogniloop on GitHub" instead of "GitHub right-pointing arrow"; wrap decorative → in aria-hidden spans; add aria-label to "Pick up where you left off" resume buttons so screen readers say "Resume [topic] — [mode] mode, [date]" instead of reading the decorative ↻ icon Unicode name; add aria-label to CurriculaPicker "See all →" link — four targeted WCAG 2.1 SC 2.4.6 / SC 4.1.2 fixes across NavBar.tsx and Landing.tsx

- 2026-06-07 UI copy polish: markdown export round headers now read "Round N · Explain · difficulty 3/5" instead of "Round N — explain (diff 3)" — capitalizes the question type, expands the jargon "diff" abbreviation, and uses "· difficulty N/5" to match the in-session difficulty tooltip style; strengths/gaps section labels in the exported markdown now read "**What you got:**" and "**Still missing:**" to match the identical labels shown in the EvalCard during the live session and in the coaching report round review, making the downloaded file consistent with the in-app vocabulary

- 2026-06-07 additional curriculum topic: expanded "Computer organization" stub in GATE CSE Theory & Compilers from a bare one-liner into a full chapter — Von Neumann vs Harvard architecture, performance metrics (CPI, MIPS, AMAT, Amdahl's Law), RISC vs CISC and addressing modes, 5-stage pipeline (IF/ID/EX/MEM/WB), structural/data/control hazards, RAW/WAR/WAW hazard types, forwarding/bypassing, load-use stall, branch prediction (1-bit, 2-bit saturating counter, BTB), SRAM vs DRAM, cache organisation (direct-mapped, k-way set-associative, fully-associative), set/tag/offset address decomposition, replacement policies (LRU/FIFO/random), write policies (write-through/write-back, write-allocate/no-write-allocate), miss types (compulsory/capacity/conflict), multi-level cache local vs global miss rates, TLB, memory-mapped vs port-mapped I/O, programmed I/O vs interrupt-driven vs DMA; a 3–5 question GATE CSE topic tested via pipeline stall count, AMAT calculation, cache address decomposition, and branch predictor state-machine tracing — previously a bare one-liner stub
...
- 2026-06-07 refactor: merge duplicate pickExample and pickRecent callbacks in Landing.tsx into a single pickTopic — both useCallback hooks had identical bodies (setTopic, setErr(null), focus input); removing the duplicate cuts 5 lines with no behavior change

- 2026-06-07 additional curriculum topic: expanded "Thermodynamics" stub in NEET Physics from a bare label to full NCERT Class 11 Ch 12 chapter coverage — zeroth law (thermal equilibrium, temperature as state function), first law ΔU = Q − W with sign conventions, work W = ∫P dV as area under P-V diagram, four processes and their key identities (isothermal: ΔU=0, W=nRT ln(V₂/V₁); adiabatic: Q=0, PV^γ=const, W=(P₁V₁−P₂V₂)/(γ−1); isochoric: W=0, Q=nCvΔT; isobaric: W=nRΔT, Q=nCpΔT), molar heat capacities Cv/Cp/γ with Mayer's relation Cp−Cv=R and γ values (monoatomic He/Ar γ=5/3; diatomic N₂/O₂ γ=7/5; triatomic H₂O γ=4/3), Carnot efficiency η=1−T₂/T₁, Carnot's theorem, refrigerator COP=T₂/(T₁−T₂), and qualitative entropy; a 4–6 question NEET Physics chapter (NCERT Class 11 Ch 12) tested via Q/W/ΔU calculations for given process type, Carnot efficiency from source/sink temperatures, γ identification for gas type, and Mayer's relation — previously a bare stub in the NEET Physics pack

- 2026-06-07 a11y: add roving tabindex + arrow-key navigation to the provider preference radiogroup in SettingsView — Auto/Groq/Pollinations buttons now follow the WCAG ARIA radiogroup pattern (only the active option is in the tab order; ← → ↑ ↓ cycle between options), matching the identical fix applied to ModePicker on 2026-06-06; mouse and touch behavior unchanged
...
- 2026-06-06 additional curriculum topic: expanded "Learning and memory" stub in MCAT Psych/Soc from a bare label to full MCAT-level chapter coverage — classical conditioning (acquisition, extinction, spontaneous recovery, generalization, discrimination, higher-order; Garcia taste-aversion effect), operant conditioning (positive/negative reinforcement vs punishment, FR/VR/FI/VI schedules and extinction resistance, shaping), observational learning (Bandura, Bobo doll, four stages), cognitive learning (latent learning, insight learning, learned helplessness); memory systems (sensory → working/STM → LTM); LTM taxonomy (declarative: episodic vs semantic; non-declarative: procedural, priming, conditioned responses); levels of processing (structural < phonological < semantic, self-reference effect), encoding specificity, context/state-dependent memory; retrieval types (recall vs recognition vs relearning), tip-of-the-tongue; forgetting (decay, retroactive vs proactive interference, repression, source monitoring errors); memory errors (misinformation effect, Loftus, false memory, weapon focus, schema distortion); biological substrates (hippocampus consolidation, H.M. anterograde amnesia, amygdala emotional memory, cerebellum procedural, PFC working memory, LTP as cellular mechanism); a 5–8 question MCAT Psych/Soc section topic previously a bare stub

- 2026-06-06 additional curriculum topic: expanded "2D arrays" stub in AP CSA from a bare label to full AP Unit 8 chapter coverage — declaration and initialization (int[][] grid = new int[rows][cols], explicit initializer lists), row/column access (grid[row][col], 0-based, ArrayIndexOutOfBoundsException), dimension queries (grid.length for rows, grid[0].length for columns), nested for-loop traversal in row-major and column-major order, enhanced for loop with 2D arrays, common algorithms (sum all elements, find max/min, search, count, transpose, row/column aggregation), jagged arrays (per-row length with grid[r].length), passing 2D arrays to methods; a 3–5 question AP CSA Unit 8 topic previously a bare stub

- 2026-06-06 additional curriculum topic: expanded GOC (General Organic Chemistry) stub in JEE Chemistry from a bare 4-word label to full chapter coverage — inductive effect (+I/−I groups, acidity applications), mesomeric/resonance effect (+M/−M, EAS directing effects), hyperconjugation (no-bond resonance, carbocation/alkene stability order), reactive intermediates (carbocations, carbanions, free radicals, carbenes), electrophile/nucleophile classification, pKa reference scale, aromatic stability (Hückel rule) — a 3–5 question JEE Main chapter (NCERT Class 11 Ch 12–13) that underpins all subsequent JEE organic chemistry; previously a bare stub
...
- 2026-06-06 additional curriculum topic: added "Human health and disease" to NEET Biology (12→13 topics) — common infectious diseases (typhoid: Salmonella typhi + Widal test; pneumonia: Streptococcus pneumoniae fills alveoli; malaria: Plasmodium 4 species, female Anopheles vector, erythrocytic schizogony, falciparum → cerebral malaria; filariasis: Wuchereria bancrofti + Culex; ringworm: Microsporum/Trichophyton fungi); innate immunity (physical/chemical/cellular barriers, complement, interferon, NK cells) vs acquired immunity (humoral: B-cells/plasma cells/antibodies/memory-B; cell-mediated: helper T CD4⁺/cytotoxic T CD8⁺); active (vaccination/infection → memory cells, slow onset) vs passive (ATS, colostrum, maternal IgG via placenta, immediate but temporary) immunity; allergies (IgE/mast-cell/histamine, type I hypersensitivity); autoimmunity (rheumatoid arthritis, type 1 diabetes, SLE); HIV/AIDS: retrovirus, gp120 binds CD4⁺ T-cells, reverse transcriptase → proviral DNA, progressive CD4 destruction → AIDS when count < 200 cells/μL, transmission routes (sexual/needle/blood/vertical), ELISA diagnosis, ART but no vaccine; cancer: proto-oncogene → oncogene mutation, tumour suppressors (p53/Rb), benign vs malignant/metastatic, types (carcinoma/sarcoma/lymphoma/leukaemia), carcinogens (UV, ionising radiation, chemical, oncogenic viruses HPV/EBV/HBV), diagnosis (biopsy/MRI/CT/PET), treatment (surgery/radiotherapy/chemotherapy/immunotherapy); drugs/alcohol: opioids (heroin/morphine), cannabinoids (THC), cocaine (dopamine reuptake block), tobacco (nicotine + tar + CO → lung cancer/COPD), adolescent vulnerability; a 3–5 question NEET chapter (NCERT Class 12 Ch 8) tested via disease-vector-pathogen matching, innate vs acquired and active vs passive immunity distinction, HIV CD4⁺ mechanism and ELISA diagnosis, cancer types and carcinogen categories — previously absent from the NEET Biology pack

- 2026-06-06 UI copy polish: update root layout meta description and keywords to include all 8 curriculum packs (AP CS, ML Fundamentals, System Design, Economics were missing from both description and keywords array)
...
- 2026-06-05 additional curriculum topic: added "Labor markets and income distribution" to Economics Microeconomics (6→7 topics) — labor demand (MRPL = MR × MPL, profit-maximizing hiring rule wage = VMPL, derived demand shifts from technology/product prices/worker quality); labor supply: reservation wage, labor-leisure trade-off, compensating wage differentials, backward-bending supply curve; monopsony (single buyer of labor: MFC > wage curve, underhires at lower wage than competitive equilibrium, deadweight loss); minimum wage: standard competitive model (unemployment = QL* − QD when floor above equilibrium) vs Card-Krueger natural experiment (New Jersey vs Pennsylvania fast food — near-zero disemployment effect consistent with employer monopsony power); human capital theory (Becker): education as investment in productivity, Mincer wage equation (log W = α + βS + γEXP); signaling model (Spence): education as costly signal of innate ability, separating equilibrium; wage discrimination: taste-based (Becker — prejudiced employers pay premium to avoid minority workers, driven out by competition long-run) vs statistical discrimination (group-average proxies, persists without prejudice, self-fulfilling); functional income distribution — labor vs capital shares, Cobb-Douglas; personal distribution: Lorenz curve, Gini coefficient (U.S. ≈ 0.48 pre-tax / ≈ 0.39 post-transfers), EITC as wage subsidy for low earners; union collective bargaining: monopoly union model, bilateral monopoly with monopsony employer, union wage premium ~10–15%; a full AP Micro Unit 4 topic (factor markets and income distribution) and core CFA Level I Economics section previously absent from the Microeconomics subject

- 2026-06-05 additional curriculum topic: added "Design a video streaming platform (Netflix / YouTube)" to System Design Design Problems (7→8 topics) — transcoding pipeline (FFmpeg, multi-bitrate ladder H.264/H.265/AV1), adaptive bitrate streaming (HLS/MPEG-DASH, client-side ABR), CDN delivery with origin shield and Range-header seeking, metadata vs blob storage split, Redis view-count hot-path, recommendation via collaborative filtering + Kafka→Flink engagement signals, live streaming LL-HLS; previously the most prominent missing design problem in a system design interview curriculum

- 2026-06-05 additional curriculum topic: added "Neural control and coordination" to NEET Biology (11→12 topics) — neuron structure (soma, dendrites, myelinated axon with Schwann cells and nodes of Ranvier — saltatory vs continuous conduction); resting membrane potential −70 mV (Na⁺/K⁺ ATPase: 3 Na⁺ out / 2 K⁺ in); action potential: threshold ~−55 mV → depolarization (voltage-gated Na⁺ in, +30 mV) → repolarization (K⁺ out) → hyperpolarization → absolute refractory period; all-or-none principle; chemical synapse: Ca²⁺ influx → vesicle fusion → NT into cleft → EPSP (glutamate/ACh) or IPSP (GABA/glycine); key NTs: ACh (NMJ + parasympathetic), dopamine (reward — Parkinson's on loss), serotonin (mood), norepinephrine (sympathetic); human brain — forebrain: cerebrum (4 lobes: frontal/motor, parietal/somatosensory, temporal/auditory, occipital/visual), corpus callosum, thalamus (sensory relay), hypothalamus (homeostasis + pituitary link); midbrain (visual/auditory reflexes); hindbrain: pons (respiration), medulla oblongata (vital centres: cardiac, vasomotor, respiratory), cerebellum (coordination and balance); spinal cord: H-shaped grey matter (dorsal horn sensory, ventral horn motor) + white matter tracts; reflex arc: receptor → afferent (dorsal root ganglion) → spinal interneuron → efferent (ventral horn) → effector; monosynaptic knee-jerk vs polysynaptic withdrawal; eye: cornea → iris/pupil → lens (accommodation: ciliary muscles) → retina — rods (rhodopsin, dim light, periphery) and cones (iodopsin, colour vision, fovea centralis); blind spot at optic disc; ear: tympanic membrane → ossicles (malleus → incus → stapes) → oval window → cochlea (basilar membrane + organ of Corti + hair cells → cochlear nerve); semicircular canals (angular acceleration), utricle/saccule (static equilibrium); a 6–8 question NEET chapter (NCERT Class 11 Ch 21) tested via action potential steps, neurotransmitter identification, brain lobe functions, reflex arc sequence, and rods vs cones distinction — previously absent from the NEET Biology pack
...
- 2026-06-05 additional curriculum topic: added "d and f block elements" to JEE Chemistry (13→14 topics) — position in periodic table (d-block Groups 3–12, f-block lanthanoids/actinoids); electronic configuration of 3d series with exceptions Cr [Ar]3d⁵4s¹ and Cu [Ar]3d¹⁰4s¹ (half-filled/fully-filled 3d stability); general properties: variable oxidation states (ns and (n−1)d close in energy), coloured ions (d-d transitions: Cu²⁺ blue, Ni²⁺ green, MnO₄⁻ charge transfer; d⁰/d¹⁰ colourless), magnetic moment μₑff = √(n(n+2)) B.M., catalytic activity (Fe Haber, Pt Ostwald, V₂O₅ Contact), alloy formation (brass/bronze/steel), interstitial compounds (TiC/TiN — hard, high m.p., non-stoichiometric), complex formation; KMnO₄: preparation MnO₂ → K₂MnO₄ → KMnO₄, oxidising in acidic medium (MnO₄⁻ + 8H⁺ + 5e⁻ → Mn²⁺, +1.51 V), neutral (→ MnO₂ brown ppt), strongly alkaline (→ MnO₄²⁻ green); K₂Cr₂O₇: preparation from chromite ore via Na₂CrO₄, Cr₂O₇²⁻ + 14H⁺ + 6e⁻ → 2Cr³⁺ + 7H₂O, dichromate-chromate pH equilibrium; lanthanoids — [Xe]4f^(1–14) configuration, predominantly +3 (Ce⁴⁺ exception), lanthanoid contraction → Zr/Hf nearly identical radii; actinoids — all radioactive, wider OS range (+3 to +7) due to 5f/6d/7s near-degeneracy; a 2–4 question JEE Main chapter (NCERT Class 12 Ch 8) tested via Cr/Cu configuration exceptions, μₑff calculation, KMnO₄ medium-dependent reduction, K₂Cr₂O₇ reaction, interstitial compound vs alloy distinction, and lanthanoid contraction — previously absent from the JEE Chemistry pack

- 2026-06-05 tiny feature: concept tracker panel is now collapsible on mobile — a `›` chevron button appears in the panel header on small screens (hidden on lg+); tapping it collapses or expands the concept list, freeing vertical space for the answer textarea; panel starts expanded; aria-expanded + aria-controls wired for accessibility; on desktop the panel stays sticky and always-visible as before

- 2026-06-05 keyboard shortcut: press T during the answering phase to toggle the LaTeX math syntax panel — same as clicking the ƒ button; aria-keyshortcuts="t" added to the button, title updated to mention the shortcut, T listed in ShortcutsModal Session section; useful for JEE/NEET/GATE/MCAT sessions where inline LaTeX is frequently needed

- 2026-06-05 additional curriculum topic: added "Alcohols, phenols and ethers" to NEET Chemistry (9→10 topics) — alcohol classification (1°/2°/3°), H-bonding physical properties, preparation (hydration, NaBH₄/LiAlH₄ reduction, Grignard), reactions (dehydration at 140°C→ether/170°C→alkene, PCC vs KMnO₄ oxidation, Lucas test: ZnCl₂/conc.HCl — 3° immediate cloudy/2° ~5 min/1° no reaction at RT, Fischer esterification), phenol acidity via resonance-stabilised phenoxide (pKa ≈10 vs alcohol pKa 16 vs acid pKa 5), EAS reactions (tribromophenol precipitate, Kolbe-Schmitt salicylate synthesis, Reimer-Tiemann o-hydroxybenzaldehyde), Williamson synthesis (SN2, primary alkyl halide only), ether cleavage by HI/HBr, epoxide ring-opening (acid: more-substituted carbon; base/nucleophile: less-substituted with inversion); a 3–5 question NEET Chemistry chapter (NCERT Class 12 Ch 11) tested via Lucas test reactivity order, tribromophenol/Kolbe-Schmitt/Reimer-Tiemann named reactions, phenol acidity comparisons, and Williamson synthesis constraints — previously absent from the pack despite Haloalkanes (Ch 10) and Aldehydes (Ch 12) already being present

- 2026-06-05 additional curriculum topic: added "Microbes in human welfare" to NEET Biology (10→11 topics) — household fermented foods (Lactobacillus in curd, Saccharomyces in bread/beer/wine, Propionibacterium in Swiss cheese, Penicillium in Roquefort); industrial products (citric acid from Aspergillus niger, bioethanol, antibiotics: penicillin/streptomycin/chloramphenicol; drugs: cyclosporin A from Trichoderma polysporum, lovastatin/statins from Monascus purpureus, streptokinase from Streptococcus); sewage treatment (primary physical → secondary biological via activated sludge and trickling filters, BOD defined and used to measure water pollution, anaerobic digesters → biogas); biocontrol agents (Bt spore-crystal preparations, Trichoderma fungi, Baculovirus/NPVs, Gambusia fish against mosquito larvae); biofertilizers (Rhizobium in legume root nodules with leghaemoglobin O₂-buffer, free-living Azotobacter/Azospirillum/cyanobacteria, mycorrhizal Glomus fungi for phosphorus absorption); a 2–4 question NEET chapter (NCERT Class 12 Ch 10) tested via sewage BOD, cyclosporin/statin source organisms, Rhizobium nodule biology, and mycorrhizal role — previously absent from the NEET Biology pack as a standalone topic

- 2026-06-04 additional curriculum topic: expanded "Optics" and "Modern physics" in NEET Physics from bare stubs to full NCERT Class 12 chapters — Optics covers ray optics (mirror/lens formulae, TIR, optical fibres, Brewster's angle) and wave optics (YDSE fringe width, single-slit diffraction, Malus' law, polarization); Modern physics covers dual nature (Einstein's photoelectric equation, de Broglie wavelength, Davisson-Germer, Heisenberg uncertainty), atoms (Rutherford's α-scattering, Bohr's model energy levels E_n = −13.6/n² eV, all 5 spectral series with Rydberg formula), and nuclei (mass defect, BE/A curve, α/β/γ decay laws, half-life T₁/₂ = 0.693/λ, fission and fusion); a 14–22 question combined NEET Physics section (NCERT Class 12 Ch 9–13) now fully covered — previously two single-word placeholder entries

- 2026-06-04 additional curriculum topic: added "Aldehydes, ketones and carboxylic acids" to NEET Chemistry (8→9 topics) — Tollens'/Fehling's/Benedict's identification tests for aldehydes vs ketones, iodoform reaction (CH₃COR + I₂/NaOH → CHI₃ yellow precipitate — identifies methyl ketones and secondary alcohols with adjacent CH₃), aldol condensation (enolizable carbonyls + dilute NaOH → β-hydroxy carbonyl → dehydration → α,β-unsaturated product), Cannizzaro reaction (non-enolizable aldehydes + conc. NaOH → disproportionation: one oxidised to carboxylate, one reduced to alcohol), Rosenmund reduction for aldehydes (RCOCl + H₂/Pd-BaSO₄), Clemmensen vs Wolff-Kishner for C=O → CH₂ (acidic vs basic conditions), Hell-Volhard-Zelinsky α-halogenation of carboxylic acids, carboxylic acid acidity order (Cl₃CCOOH > HCOOH > PhCOOH > CH₃COOH — inductive effect), NaHSO₃ bisulfite adduct (aldehydes and methyl ketones only), acid derivative reactivity order (acid chlorides > anhydrides > esters > amides); a 4–6 question NEET Chemistry chapter (NCERT Class 12 Ch 12) tested via identification reagents, aldol/Cannizzaro criteria, and acidity trends — previously absent from the pack
...
- 2026-06-04 additional curriculum topic: expanded "Biomolecules and polymers" in JEE Chemistry from a bare stub to a full NCERT Class 12 Ch 15 polymer chapter — addition polymerisation (free-radical mechanism, Ziegler-Natta TiCl₄/AlEt₃ for linear HDPE), condensation polymerisation (nylon 6,6 from hexamethylenediamine + adipic acid, nylon 6 by caprolactam ring-opening, Dacron/terylene from ethylene glycol + terephthalic acid, Bakelite 3D-crosslinked thermoset, glyptal alkyd resin), thermoplastics vs thermosets distinction, natural rubber (cis-1,4-polyisoprene) vulcanization with 3–5% S forming sulfur cross-links, synthetic rubbers (Buna-S/SBR for tyres, Buna-N/nitrile rubber oil-resistant, neoprene flame-resistant), biodegradable polymers (PHBV bacterial copolymer, nylon 2-nylon 6), number-average Mn vs weight-average Mw and polydispersity index PDI; a 2–3 question JEE Main chapter previously a single-line stub in the JEE Chemistry pack

- 2026-06-04 perf: memoize "Pick up where you left off" chips in Landing — extract the three recent-topic buttons into a stable RecentTopics memo component with a useCallback onPick handler, so they skip re-renders on every topic-input keystroke (the recentTopics array reference is set once on mount and never changes); matches the CurriculaPicker / FeatureGrid / StatsPanel memo pattern already in Landing

- 2026-06-04 additional curriculum topic: expanded "Coordination compounds" in JEE Chemistry from a bare stub to a full NCERT Class 12 Ch 9 topic — Werner's primary vs secondary valence, coordination entity notation, ligand types (monodentate, bidentate, polydentate/chelating EDTA⁴⁻, ambidentate NO₂⁻/SCN⁻, bridging), chelate effect, complete IUPAC nomenclature rules with examples ([Co(NH₃)₆]Cl₃, K₂[PtCl₆]), structural isomers (linkage, ionisation, solvate/hydrate), stereoisomers (geometric cis/trans and fac/mer, optical Δ/Λ enantiomers), Crystal Field Theory (t₂g vs eg splitting in octahedral field, Δo), spectrochemical series (I⁻ → CO ordering), high-spin vs low-spin configurations for d⁴–d⁷, CFSE, colour from d-d transitions, magnetic moment μₑff = √(n(n+2)) B.M., biological roles (haemoglobin Fe²⁺, chlorophyll Mg²⁺, vitamin B₁₂ Co³⁺), and industrial applications (Wilkinson's catalyst, cisplatin anticancer drug, Mond process for Ni, cyanide leaching for Au/Ag); a 3–5 question JEE Main chapter previously absent as a bare placeholder in the JEE Chemistry pack
